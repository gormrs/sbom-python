...
class TestSbom(unittest.TestCase):
    ...
    def test_write_json(self):
        write_json("test_dependency", "1.0.0", "npm", "/path/to/repo",
                   "1234567890abcdef", "https://www.npmjs.com/package/test_dependency")
    ...
    def test_write_csv(self):
        write_csv("test_dependency", "1.0.0", "npm", "/path/to/repo",
                  "1234567890abcdef", "https://www.npmjs.com/package/test_dependency")
    ...
    def test_get_npm_link(self):
        assert get_npm_link(
            "test_dependency") == "https://www.npmjs.com/package/test_dependency"

    def test_get_pypi_link(self):
        assert get_pypi_link(
            "test_dependency") == "https://pypi.org/project/test_dependency"

    def create_temp_git_repo(self):
        temp_dir = tempfile.mkdtemp()
        subprocess.run(["git", "init"], cwd=temp_dir, check=True)

        with open(os.path.join(temp_dir, "temp_file.txt"), "w", encoding="utf-8") as temp_file:
            temp_file.write("Temporary content")

        with open(os.path.join(temp_dir, "requirements.txt"), "w", encoding="utf-8") as pip_file:
            pip_file.write("test_package==1.0.0\n")

            # Create a fake repository with a package.jsonpip_file file
        with open(os.path.join(temp_dir, "package.json"), "w", encoding="utf-8") as npm_file:
            json.dump({"dependencies": {"test_package": "1.0.0"}}, npm_file)

        subprocess.run(["git", "add", "temp_file.txt"], cwd=temp_dir, check=True)
    ...
    def test_get_latest_commit(self):
        ...
        commit_hash = subprocess.check_output(
            ["git", "log", "--format=%H", "-n", "1"], cwd=temp_repo_path).decode("utf-8").strip()
